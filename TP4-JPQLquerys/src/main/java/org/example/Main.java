...
*/            Cliente cliente1 = new Cliente("20429759650", "Consumidor final");
   ...
   */         Cliente cliente2 = new Cliente("20439869751", "Consumidor final");
     /*       em.persist(cliente2);
          ...
          em.persist(factura2);
*/
            Factura factura4 = new Factura(FuncionApp.generateRandomDate(), LocalDateTime.now(), FuncionApp.generateRandomDate(), 5271416458273461L, 4);

            Cliente cliente = em.find(Cliente.class, 2L);
            factura4.setCliente(cliente);
            em.persist(factura4);
            em.getTransaction().commit();
            FacturaManager facturaManager  = new FacturaManager(false);
            ClienteManager clienteManager = new ClienteManager(false);
...
//8
            System.out.println("Articulo: " + facturaManager.getArticuloMasCaroXId(1));
            //9
            System.out.println("Cantidad de facturas: " + facturaManager.getTotalFacturas());
            //10
            for (Factura a : facturaManager.getFacturasMayorAPrecio(200)){
                System.out.println("ID: " + a.getId() + "\n" );
            }
            //11
            for (Factura a : facturaManager.getFacturasQueContienenArticulo(art1)){
                System.out.println("Factura ID: " + a.getId());
            }
            //12
